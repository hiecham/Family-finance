...
@override Widget build(BuildContext context) { final pages = [ DashboardPage(entries: _entries, onRefresh: _refresh), AddEntryPage(onAdd: _addEntry), ExpensesPage(entries: _entries, onDelete: _delete), SavingsPage(entries: _entries), InvestmentsPage(entries: _entries), GoalsPage(), SettingsPage(onClear: () async { await Store.save([]); setState(() => _entries = []); }), ];
...
// ===== Purchase Goals (اهداف خرید) =====

class PurchaseGoal { final String id; String title; String? note; bool done; PurchaseGoal({ required this.id, required this.title, this.note, this.done = false, });

Map<String, dynamic> toJson() => { 'id': id, 'title': title, 'note': note, 'done': done, };

factory PurchaseGoal.fromJson(Map<String, dynamic> j) => PurchaseGoal( id: j['id'], title: j['title'] ?? '', note: j['note'], done: j['done'] ?? false, ); }

class GoalsStore { static const _k = 'purchase_goals'; static Future<List<PurchaseGoal>> load() async { final sp = await SharedPreferences.getInstance(); final raw = sp.getString(_k); if (raw == null) return []; final list = (jsonDecode(raw) as List).cast<Map<String, dynamic>>(); return list.map(PurchaseGoal.fromJson).toList(); }
